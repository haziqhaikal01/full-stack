...
# # Define available locations (InfluxDB tag keys)
# LOCATIONS = ["M13 9PL", "S23 9RW"]  # ✅ Tag keys for Manchester & Scotland

# # Function to continuously write frequency data with tag-based locations
# def write_continuous_frequencies():
#     i = 1  # Counter for records

#     try:
#         while True:  # Infinite loop for continuous writing
#             for location in LOCATIONS:  # ✅ Iterate through locations
#                 frequency = round(np.random.uniform(49.7, 50.3), 3)  # Generate random frequency
#                 timestamp = int(time.time() * 1e9)  # Convert current time to nanoseconds

#                 # Create InfluxDB point with tag-based location
#                 point = (
...
#             time.sleep(1)  # ✅ Move outside the loop to avoid delays between locations
...
def write_fixed_frequencies():
    ...
    try:
        while True:  # Infinite loop for continuous writing
            ...
            time.sleep(1)  # ✅ Wait 1 second before writing next batch
